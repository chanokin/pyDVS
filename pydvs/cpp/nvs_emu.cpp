...
bool PyDVS::init(const int cam_id, const float thr, const float relaxRate, 
        const float adaptUp, const float adaptDown, const float leakProb){
    ...
    setAdapt(relaxRate, adaptUp, adaptDown, thr, leakProb);
    _initMatrices(thr);
    return success;
}
...
bool PyDVS::init(const char* filename, const float thr, const float relaxRate, 
    ...
    _get_fps();
    // set output format as 32-bit floating point with a single channel
    _cap.set(cv::CAP_PROP_FORMAT, CV_16FC1);
    setAdapt(relaxRate, adaptUp, adaptDown, thr, leakProb);
    _initMatrices(thr);
...
bool PyDVS::update(){
    ...
    cv::parallel_for_(cv::Range(0, _gray.rows), _nvsOp);
