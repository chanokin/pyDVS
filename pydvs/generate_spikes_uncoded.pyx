...
def local_inhibition(numpy.ndarray[DTYPE_t, ndim=2] spikes,
  ...
  """ Searches for the largest change in an area and removes spiking mark 
      ...
      :param abs_diff:    Absolute value of the intensity change
...
def update_reference_rate(numpy.ndarray[DTYPE_t, ndim=2] abs_diff,
  ...
  """
    ...
    :param threshold:       How much intensity has to change to mark a pixel as spiking
...
def update_reference_rate_adpt(numpy.ndarray[DTYPE_t, ndim=2] abs_diff,
  ...
  """
    ...
    :param spikes:          Pixels marked as spiking
    :param ref_frame:       Previous reference frame
    :param threshold:       How much intensity has to change to mark a pixel as spiking in
...
def update_reference_time_binary_raw(numpy.ndarray[DTYPE_t, ndim=2] abs_diff,
  ...
  """
    ...
    Binary encoding of the raw intensity difference value, using only num_spikes active bits
...
def update_reference_time_binary_thresh(numpy.ndarray[DTYPE_t, ndim=2] abs_diff,
  ...
  """
    ...
    Binary encoding of the number of thresholds rebased by the intensity difference, 
...
def render_frame(numpy.ndarray[DTYPE_t, ndim=2] spikes, 
  ...
  """
    ...
    source. Red means a negative change in intensity, Green a positive one.
...
def split_spikes(numpy.ndarray[DTYPE_t, ndim=2] spikes,
  ...
  """
    ...
    :returns negative: Pixels that spiked with a negative change in intensity
    :returns positive: Pixels that spiked with a positive change in intensity
    :returns global_max: Value of the largest change in intensity
...
cdef DTYPE_t spike_to_key(DTYPE_t row, DTYPE_t col, 
    ...
    """
      *nogil allows parallel computation
      ...
      :param is_pos_spike: A flag to indicate whether the change in intensity was positive
                           or negative
...
def make_spike_lists_rate(numpy.ndarray[DTYPE_U16_t, ndim=2] pos_spikes,
  ...
  cdef Py_ssize_t spike_idx, pix_idx
  cdef list spike_key = list()
